...
A_filename = dir('input_data/drum_separation_result/*.wav');
...
for i = 1:length(A_filename)
    % 2-1. read audio of A
    [A_x,fs] = audioread(['input_data/drum_separation_result/',A_filename(i).name]);
...
[B_x,~] = audioread('input_data/wet_mix.wav');
...
audiowrite('output/pannign_imitation_result.wav',y,fs);
clear y1_sum y2_sum