%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Name: Dual channel score informed NMF for drum separation% Date: April 2021
%
% Description :
% This program is for drum separation with score informed NMF and NMFD. The sources are
...
inpPath = 'input_data/';
%%
% 1-1 read target audio
[x_R,fs] = audioread([inpPath, 'OHR.wav']);
[x_L,~] = audioread([inpPath, 'OHL.wav']);
...
% 4-4. NMF initial H (score informed)
paramActivations.numComp = numComp;
paramActivations.numFrames = numFrames;
paramActivations.onsets = importdata([inpPath, 'onsets.txt']);
paramActivations.drums = importdata([inpPath, 'drums.txt']);
...
%% dual channel score informed NMF
...
paramNMF.initH = HL;
[WL,HL] = NMF(A_L,paramNMF);
disp('complete dual channel score informed NMF')
clear paramNMF
...
% 11-4 save plot
saveas(gcf,'output/right_channel.png')
close(gcf)
...
clear paramVis
% 8-4 save plot
saveas(gcf,'output/left_channel.png')
close(gcf)
clear A_L WL HL
    ...
    switch char(instrument)
        case 'BD'
            if dictionaryW_length{2,1} ~= 0
                ...
                BD = BD/max(abs(BD));
                audiowrite('output/1_BD.wav',BD, fs);
        ...
        case 'SD'
            if dictionaryW_length{2,2} ~= 0
                ...
                SD = SD/max(abs(SD));
                audiowrite('output/2_SD.wav',SD, fs);
        ...
        case 'HH'
            if dictionaryW_length{2,3} ~= 0
                ...
                HH = HH/max(abs(HH));
                audiowrite('output/3_HH.wav',HH, fs);
        ...
        case 'Tom1'
            if dictionaryW_length{2,4} ~= 0
                ...
                T1 = T1/max(abs(T1));
                audiowrite('output/4_T1.wav',T1, fs);
        ...
        case 'Tom2'
            if dictionaryW_length{2,5} ~= 0
                ...
                T2 = T2/max(abs(T2));
                audiowrite('output/5_T2.wav',T2, fs);
        ...
        case 'Floor Tom'
            if dictionaryW_length{2,6} ~= 0
                ...
                FT = FT/max(abs(FT));
                audiowrite('output/6_FT.wav',FT, fs);
        ...
        case 'Crash'
            if dictionaryW_length{2,7} ~= 0
                ...
                audiowrite('output/7_Crash.wav',Crash, fs);
        ...
        case 'Ride'
            if dictionaryW_length{2,8} ~= 0
                ...
                audiowrite('output/8_Ride.wav',Ride, fs);
